package codes.chia7712.nmmst.processor;
...
/**
 * Calculates the fusion range by linear format. We decrease the RGB value for
 * the image edge. The processed edges for LU image are as follows:
 * <p>
 * |-------------------------------|-------------------------------| | *|* | |
 * LU *|* RU | | *|* |
 * |*******************************|*******************************|
...
public class LinearProcessor implements FrameProcessor {
  ...
  /**
   * Fuses the edge for specified image. We rewrite the RGB value in the source
   * image instead of cloneing image.
   *
  ...
  /**
   * The factor of linear progrmmming is used for fusing the image edge. Each
   * argument is represented as following: min scale max scale |-------------|
   * overlay
   */
  ...
  /**
   * Calculates the RGB value for fusing the edge. We use linear programming to
   * evaluate the value.
   */
  private static class LinearEquationInTwo {
    ...
    /**
     * Constructs a linear equation for two specified coordinates. The x
     * represents the pixel location (x or y) and the y represents the weight
     * for RGB value.
     *

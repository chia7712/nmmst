 ...
 * this movie should be skiped. So we can control the play flow by setting the
 * skip flag for each movie.
 *
 * @see MediaWorker
 */
public final class MovieInfo {
  ...
  /**
   * Constructs the movie order by cloning the file list. The initial order is
   * equal with order in list.
   *
  ...
  /**
   * Returns {@code true} if this movie info contains a mapping for the
   * specified key.
   *
  ...
  /**
   * Creates a play flow with a order. The {@link PlayFlow}'s order is the same
   * as this movie's.
   *
   * @return A play flow
   */

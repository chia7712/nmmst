...
/**
 * A media controller encapsulates the details of video output, audio output and
 * the media decoder. It may invokes many threads for decodeing media, drawing
 * image and writing audio data. The straightforward methods is used for
 * executing the {@link codes.chia7712.nmmst.utils.RequestUtil.RequestType}.
 */
public interface MediaWorker {
  ...
  /**
   * Sets the movie index fo specified order. The movie index will replace the
   * old index if the order has been set.
   *
  ...
  /**
   * Pauses the {@link MovieBuffer#readFrame()} and {@link MovieBuffer#readSample()}.
   *
